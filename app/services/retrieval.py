...
def _build_retrieval_intent(
    query: str,
    section_types_override: Optional[list[str]] = None,
) -> _RetrievalIntent:
    """Build retrieval intent, optionally overriding inferred section types."""
    intent = _infer_intent(query)
    if section_types_override:
        merged = list(section_types_override)
        for st in intent.section_types:
            if st not in merged:
                merged.append(st)
        return _RetrievalIntent(section_types=merged, entities=intent.entities)
    return intent
...
def _retrieve_parent_documents_sync(
    ...
    *,
    ...
    intent = _build_retrieval_intent(query, section_types)
...
async def hybrid_retrieve(
    ...
    parent_docs = await retrieve_parent_documents(
        query,
        top_k=top_k,
        task_id=task_id,
        section_types=section_types,
