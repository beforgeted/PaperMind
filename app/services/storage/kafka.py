...
# NOTE: Import commented out until schemas are migrated
...
class KafkaProducerService:
    ...
    async def send_parse_task(self, task) -> None:
        """Publish a parse task to Kafka. The `task` should be a Pydantic model
        with a `.model_dump(mode="json")` method and a `.task_id` attribute."""
        if not self._started or self._producer is None:
            try:
                await self.start()
